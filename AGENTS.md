...
- **Single-entity module**: one table, `business_hours`. There is no child/aggregate entity (no
  `Agreement`-style sibling like `item_catalog` has) — `model.go` carries exactly one
  `model.Definition`, plus the transport-only args `Definition` for its one Op.
- **Not multi-tenant**: unlike most modules in this ecosystem, `BusinessHours` has no `tenant_id`
  field — it is one global weekly schedule for the whole composition-root app, not scoped per
  tenant. Do not add `tenant_id` as part of this rectification; that would be a domain change, not
  a harness-adoption change, and is out of scope for `docs/PLAN.md`.
- **Owns its schema**: `New()` calls `ddl.CreateTable(&BusinessHours{})` (via the `db.RawConn().(ddl.Compiler)`
  type-assertion pattern above) for its one table — no different from any other module in that
  respect, just a single `CreateTable` call instead of several.
- **`day_of_week` is unique and exhaustive by convention**: the table is meant to hold exactly 7
  rows (0=Sunday … 6=Saturday), enforced at the DB layer via `model.FieldDB{Unique: true}` on
  `day_of_week`, never in application code. As of this migration the module exposes only a
  **read** Op (`get_business_hours`) — there is no create/update/delete Op yet, so nothing in this
  repo ever writes a row; seeding/editing the 7 rows is the composition-root app's job (direct
  `orm.DB` access or a future write Op, out of scope here).
